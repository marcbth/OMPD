...
if ($conn === true){
	...
	<tr class="header">
		...
		<td>Artist</td>
		<td align="center">Open in Highresaudio</td>
		<td></td><!-- delete -->
	</tr>
	<?php
		$i = 0;
		for ($i = 0; $i < $trackList['totalNumberOfItems']; $i++) {
				...
				$extUrl = HIGHRESAUDIO_ALBUM_URL . $album_id;
	...
	<tr class="<?php echo ($i & 1) ? 'even' : 'odd'; ?> mouseover" id="track<?php echo $i; ?>" style="display:table-row;">
		...
		<td><?php if ($cfg['access_play']) {
			echo '<a href="javascript:ajaxRequest(\'play.php?action=addSelect&amp;track_id=highresaudio_' . $id . '\',evaluateAdd);" onMouseOver="return overlib(\'Add track\');" onMouseOut="return nd();"><i id="add_highresaudio_' . $id . '" class="fa fa-plus-circle fa-fw icon-small"></i></a>';
		}
		...
		<td><?php 
		if ($cfg['access_play']) {
			echo '<a id="fav_play_trackhighresaudio_' . $id . '" href="javascript:ajaxRequest(\'play.php?action=insertSelect&amp;playAfterInsert=yes&amp;track_id=highresaudio_' . $id . '\',evaluateAdd);" onMouseOver="return overlib(\'play track\');" onMouseOut="return nd();">' . html($title) . '</a>';
		}
