...
if ($action == 'add') {
	...
	if (mysqli_num_rows($query) == 0) {
		...
		'" . $maxPosition . "',
		'" . mysqli_real_escape_string($db, $cfg['favorite_id']) . "'
		)");
...
elseif ($action == 'remove') {
	if ($track_id){
		mysqli_query($db,"DELETE FROM favoriteitem WHERE track_id='" . mysqli_real_escape_string($db, $track_id) . "' and favorite_id='" . mysqli_real_escape_string($db, $cfg['favorite_id']) . "'");
	}
	else{
		$track_id_url = getTrackIdFromUrl($track_mpd_url);
		if ($track_id_url) {
			mysqli_query($db,"DELETE FROM favoriteitem WHERE stream_url LIKE '%track_id=" . mysqli_real_escape_string($db, $track_id_url) . "%' and favorite_id='" . mysqli_real_escape_string($db, $cfg['favorite_id']) . "'");
		}
		else {
			mysqli_query($db,"DELETE FROM favoriteitem WHERE stream_url = '" . mysqli_real_escape_string($db, $track_mpd_url) . "' and favorite_id='" . mysqli_real_escape_string($db, $cfg['favorite_id']) . "'");
		}
	}
...
$data['favorite_type'] = updateFavoriteStreamStatus($cfg['favorite_id']);
